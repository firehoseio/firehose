    ...
    class ConsumerApp
      def call(env)
        if websocket_request?(env)
          websocket.call(env)
        elsif short_poll_request?(env)
          http_short_poll.call(env)
        else
          http_long_poll.call(env)
        end
      end
      ...
      def http_short_poll
        @http_short_poll ||= HttpShortPoll.new
      end
      ...
      def short_poll_request?(env)
        ::Rack::Request.new(env).params['short_poll'] == 'true'
      end
      ...
      class HttpShortPoll
        ...
        def call(env)
          ...
          if method == 'GET'
            Firehose.logger.debug "HTTP short polling GET with for path #{path} with query #{env["QUERY_STRING"].inspect} and params #{req.params.inspect}"
            EM.next_tick do
              Channel.new(path).next_message.callback do |message, sequence|
                env['async.callback'].call response(200, message, response_headers(env))
              end.errback do |e|
                if e == :timeout
                  env['async.callback'].call response(204, '', response_headers(env))
                else
                  Firehose.logger.error "Unexpected error when trying to GET path #{path}: #{e.inspect}"
      ...
      class HttpLongPoll
        ...
        def call(env)
          ...
          when 'GET'
            Firehose.logger.debug "HTTP long polling GET with last_sequence #{last_sequence} for path #{path} with query #{env["QUERY_STRING"].inspect} and params #{req.params.inspect}"
            EM.next_tick do
